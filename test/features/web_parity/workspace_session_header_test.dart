...
void main() {
  ...
  testWidgets('compact context ring switches directly to the context pane', (
    tester,
  ) async {
  ...
  testWidgets('compact pane switcher uses a tighter mobile height', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1;
    ...
    expect(
      tester
          .getSize(
            find.byKey(
              const ValueKey<String>('workspace-compact-pane-switcher'),
            ),
          )
          .height,
      40,
  ...
  testWidgets('session header menu opens with the styled overflow panel', (
    ...
    expect(
      find.byKey(
        const ValueKey<String>(
          'session-header-overflow-menu-item-timeline-progress-details',
        ),
      ),
      findsNothing,
    );
